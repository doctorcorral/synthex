defmodule Synthex.Hub.Client do
  ...
  Two responsibilities:
    ...
    2. **Wait for completion**: polls `/api/master/batches/:id`
       until `status == "completed"`, then returns the aggregated
       `[%{idx, reward, landings}, ...]` results in candidate-index
       order.
  ...
  def score_bit(%__MODULE__{} = client, payload, opts \\ []) do
    ...
    with {:ok, batch_id, total_chunks} <- submit_batch(client, body),
      ...
      # Hub stores results as %{"chunk_index", "items"} entries. Items
      # are ordered within a chunk, chunks are ordered by chunk_index
      # (the order they were created from the candidates list). So
      # sorting by chunk_index + flattening recovers the original
      # candidate ordering. We then strip the leading baseline entry.
      flat =
        results
